...
// 大疆电流值发送函数
void set_moto_current(hcan_t* hcan, uint16_t id, int16_t iq2){
...
{
//	printf("cnt=%u",ptr->round_cnt);
	ptr->last_angle = ptr->angle;
	ptr->angle = (uint16_t)(rx_data[0]<<8 | rx_data[1]) ;
	ptr->speed_rpm  = (int16_t)(rx_data[2]<<8 | rx_data[3]);
	ptr->real_current = (rx_data[4]<<8 | rx_data[5])*5.f/16384.f;
